...
# ==========================================
# 5. MENU BARU: KELOLA & RIWAYAT DATA (HAPUS DATA)
# ==========================================
    ...
    if kategori_data == "📥 Riwayat Barang Masuk":
        if df_masuk.empty:
            ...
            # Tampilkan tabel data dengan index agar user bisa memilih nomor baris
            st.markdown("### Daftar Transaksi Masuk")
            
            # Membuat salinan untuk tampilan yang menyertakan Nama Barang (biar informatif)
            ...
            indeks_hapus = st.number_input("Masukkan Nomor Baris yang ingin dihapus (Dimulai dari angka 0 paling atas):", min_value=0, max_value=len(df_masuk)-1, step=1)
            
            # Tampilkan konfirmasi item sebelum dihapus
            item_terpilih = df_masuk_view.iloc[indeks_hapus]
            st.warning(f"Item yang akan dihapus: **{item_terpilih['Nama Barang']}** senilai **{item_terpilih['Jumlah Masuk']}** unit tanggal **{item_terpilih['Tanggal']}**")
            
            if st.button("🗑️ HAPUS TRANSAKSI MASUK PERMANEN", type="primary"):
                # Hapus baris berdasarkan indeks pilihan
                df_masuk = df_masuk.drop(df_masuk.index[indeks_hapus]).reset_index(drop=True)
                simpan_ke_excel(df_masuk, "Masuk")
                st.success("✔️ Transaksi masuk berhasil dihapus dari database Excel!")
                st.rerun()
        ...
        if df_keluar.empty:
            ...
            st.markdown("#### Hapus Baris Transaksi")
            indeks_hapus = st.number_input("Masukkan Nomor Baris yang ingin dihapus (Dimulai dari angka 0 paling atas):", min_value=0, max_value=len(df_keluar)-1, step=1)
            
            item_terpilih = df_keluar_view.iloc[indeks_hapus]
            st.warning(f"Item yang akan dihapus: **{item_terpilih['Nama Barang']}** sejumlah **{item_terpilih['Jumlah Keluar']}** unit ({item_terpilih['Keterangan']})")
            ...
            if st.button("🗑️ HAPUS TRANSAKSI KELUAR PERMANEN", type="primary"):
                ...
                simpan_ke_excel(df_keluar, "Keluar")
                st.success("✔️ Transaksi keluar berhasil dihapus dari database Excel!")
...
# ==========================================
# 3. MENU: CATAT BARANG MASUK (MULTI-ITEM DENGAN NOTA)
# ==========================================
elif menu == "📥 Catat Barang Masuk":
    st.subheader("Input Barang Masuk / Restock (Multi-Item)")
    ...
    if df_master.empty:
        ...
        with col_tbl:
            ...
            if st.session_state.keranjang_masuk:
                ...
                st.info("Keranjang masuk kosong. Silakan isi data nota di atas lalu klik 'Tambah ke Keranjang'.")
